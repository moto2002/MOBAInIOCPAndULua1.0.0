...
public class LuaScriptMgr
{
    ...
    private static LuaScriptMgr instance = new LuaScriptMgr();
    ...
    void OnBundleLoaded()
    {
        ...
        //#endif
        UnityEngine.Debug.Log("Lua Bundle loaded");
    ...
    public object[] DoStringFile(string fileName, string str)
    {
        if (!fileList.Contains(fileName))
        {
            return lua.DoStringFile(fileName, str);
        }

        return null;
    }

    public object[] DoStringFile(string fileName, byte[] bytes)
    {
        if (!fileList.Contains(fileName))
        {
            return lua.DoStringFile(fileName, bytes);
        }
    ...
    public static void RegisterLib(IntPtr L, string libName, Type t, LuaMethod[] regs, LuaField[] fields, Type baseType)
    {
        ...
        if (baseType != null)
        {
            ...
            if (LuaDLL.lua_isnil(L, -1))
            {
                LuaDLL.lua_pop(L, 1);
                LuaDLL.luaL_newmetatable(L, baseType.AssemblyQualifiedName);
                if(checkBaseType == null)
                    Debug.Log("checkBaseType is null");
